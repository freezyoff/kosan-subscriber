...
public class DoorLockStateResolver extends MqttMessageResolver {
    ...
    public DoorLockStateResolver(ServerService service) {
    ...
    public void onMessageArrived(MqttMessage message) {
        ...
        for (int i = 0; i < (locations == null ? 0 : locations.size()); i++) {
            ...
            for (int y = 0; y < (rooms == null ? 0 : rooms.size()); y++) {
                ...
                //update signal
                if (Crypto.md5(rooms.get(y).getId() + "").equals(targetRoomId)) {
        ...
        bundle.putString(ServerService.HANDLER_KEY_SENDER_CLASS, DoorLockStateResolver.class.getName());
